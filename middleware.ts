...
export async function middleware(req: NextRequest) {
  ...
  if (pathname.startsWith('/_next') || pathname.startsWith('/api') || pathname.startsWith('/static')) {
    ...
    console.log('[middleware] skipped asset/api path:', pathname);
    return r;
  }
  ...
  if (cookieVal && keyEnv && cookieVal === keyEnv) {
    const r = NextResponse.next();
    r.headers.set('x-mw-ran', '1');
    console.log('[middleware] allowed by cookie:', pathname);
    return r;
  }

  // If user provides ?key=... allow and set cookie (compare hashed)
  if (queryKey && keyEnv) {
    ...
    if (hashed === keyEnv) {
      ...
      console.log('[middleware] allowed by key param:', pathname);
      return res;
    }
  }

  // Redirect to entry page (keep origin path in query)
  const redirectUrl = new URL('/enter', req.url);
  redirectUrl.searchParams.set('from', pathname);
  // Debug: force block (401) if env set
  if (process.env.FORCE_MW_BLOCK === '1') {
    console.log('[middleware] FORCE_MW_BLOCK active, returning 401 for:', pathname);
  ...
  console.log('[middleware] redirecting to /enter from:', pathname);
  return r;
}
